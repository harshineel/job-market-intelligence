...
with col1:
    ...
    ).head(12).sort_values("avg_salary", ascending=True)
...
with col2:
    ...
    ).head(12).sort_values("flexibility_score", ascending=True)
...
if country == "All":
    st.warning("Set the country filter to USA or India for accurate salary data.")
...
display = company_stats[[
    "company", "openings", "avg_salary_fmt",
    "remote_pct", "hybrid_pct", "senior_pct", "flexibility_score"
]].sort_values("openings", ascending=False).rename(columns={
    "company":           "Company",
    "openings":          "Openings",
    "avg_salary_fmt":    f"Avg Salary ({symbol})",
    "remote_pct":        "Remote %",
    "hybrid_pct":        "Hybrid %",
    "senior_pct":        "Senior %",
